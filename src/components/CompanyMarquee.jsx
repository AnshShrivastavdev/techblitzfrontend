...
export function CompanyMarquee() {
  return (
    <section className="py-24 sm:py-32 bg-black border-y border-white/10 relative overflow-hidden select-none">
      ...
      <div className="absolute inset-0 pointer-events-none bg-[radial-gradient(circle_at_50%_50%,rgba(56,189,248,0.05),transparent_70%)]" />
      ...
      <div className="relative w-full overflow-hidden">
        ...
        {/* Marquee Track */}
        <div className="company-marquee-track flex w-max will-change-transform hover:[animation-play-state:paused]">
          {[...COMPANY_LOGOS, ...COMPANY_LOGOS].map((company, index) => (
            <div
              key={`${company.name}-${index}`}
              className="flex items-center gap-4 mx-4 sm:mx-6 px-6 py-4 rounded-xl border border-white/10 bg-neutral-950/80 hover:border-cyan-400/40 hover:bg-neutral-900/90 transition-all duration-300 group backdrop-blur-md cursor-default shadow-sm"
            >
              {/* Logo Frame with contrast protection */}
              ...
              <div className="flex flex-col text-left">
                <span className="text-xs font-bold font-mono text-white tracking-wider group-hover:text-cyan-300 transition-colors">
                  {company.name}
                </span>
