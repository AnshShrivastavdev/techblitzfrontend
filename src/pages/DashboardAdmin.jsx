...
export default function DashboardAdmin() {
  ...
  const [module, setModule] = useState('overview'); // 'overview' | 'users' | 'workshops' | 'attendance' | 'certificates' | 'faqs'
  ...
  const handleUpdateStatus = (wsId, status) => {
  ...
  const downloadCertPDF = (cert) => {
    ...
    doc.setTextColor(56, 189, 248);
    doc.setFontSize(24);
    doc.text(cert.recipientName, 148.5, 98, { align: 'center' });
    doc.setTextColor(226, 232, 240);
    doc.setFontSize(14);
    doc.text(`"${cert.workshopTitle}"`, 148.5, 120, { align: 'center' });
    doc.setTextColor(100, 116, 139);
    doc.setFontSize(10);
    doc.text(`Certificate ID: ${cert.id}`, 40, 165);
    doc.text(`Issue Date: ${new Date(cert.issueDate).toLocaleDateString()}`, 200, 165);
              ...
              <form onSubmit={handleCreateWorkshop} className="dash-profile-form" style={{ marginBottom: 28 }}>
                ...
                <div className="dash-form-row">
                  <div className="dash-form-group">
                    ...
                    <select
                      ...
                      onChange={(e) => setNewWs({ ...newWs, category: e.target.value })}
                ...
                <div className="dash-form-row">
                  <div className="dash-form-group">
                    ...
                    <input
                      type="datetime-local"
                      value={newWs.scheduledAt}
                      onChange={(e) => setNewWs({ ...newWs, scheduledAt: e.target.value })}
                    />
                  </div>
                  <div className="dash-form-group">
                    ...
                    <input
                      type="number"
                      value={newWs.durationMinutes}
                      onChange={(e) => setNewWs({ ...newWs, durationMinutes: Number(e.target.value) })}
                    />
                  </div>
                </div>
                  ...
                  >
                    <div className="dash-card" style={{ height: '100%', margin: 0 }}>
                      ...
                      <div className="dash-card__meta">
                        <div>
                          <Clock size={14} /> {ws.durationMinutes || 90} min
                        </div>
                        <div>
                          <Calendar size={14} />{' '}
                          {ws.scheduledAt
            ...
            <div className="dash-grid-cards">
              {certificates.map((c) => (
                <div key={c.id} className="dash-cert-card">
                  ...
                  <div className="dash-cert-code">ID: {c.id}</div>
                  <button
