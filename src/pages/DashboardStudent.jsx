...
export default function DashboardStudent() {
  ...
  const [activeTab, setActiveTab] = useState('workshops'); // 'workshops' | 'live' | 'certificates' | 'profile'
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      // Find if student is registered for a workshop that is currently live
  ...
  const handleManualPing = (workshopId) => {
  ...
  const downloadCertificate = (cert) => {
    ...
    doc.text('SPACE SCIENCE & EVENT MANAGEMENT NETWORK', 148.5, 45, { align: 'center' });
    ...
    doc.setTextColor(56, 189, 248);
    doc.setFontSize(24);
    doc.text(cert.recipientName || user.name, 148.5, 98, { align: 'center' });
    ...
    doc.setFontSize(16);
    doc.setFont('helvetica', 'bold');
    doc.text(`"${cert.workshopTitle}"`, 148.5, 124, { align: 'center' });

    // Issue Date & ID
    doc.setTextColor(100, 116, 139);
    doc.setFontSize(10);
    doc.text(`Issue Date: ${new Date(cert.issueDate).toLocaleDateString()}`, 40, 160);
    doc.text(`Certificate ID: ${cert.id}`, 40, 168);
    doc.text(`Verification: Verified Authenticity`, 200, 160);
    doc.text(`Authorized Signatory: Cosmos Command`, 200, 168);

    doc.save(`TechBlitz_Certificate_${cert.id}.pdf`);
  };
      ...
      <header className="dash-header">
        ...
        <div className="dash-header__user">
          ...
          <div className="dash-user-info">
            ...
            <span className="dash-user-role">🎓 Student • {user?.branch || 'Cosmic Explorer'}</span>
          </div>
          <button
            onClick={() => {
              logout();
                  ...
                  >
                    <div className="dash-card" style={{ height: '100%', margin: 0 }}>
                      <div className="dash-card__top">
                        ...
                        <span className="dash-category-tag">{ws.category || ws.track || 'Cosmos Track'}</span>
                      </div>
                      ...
                      <div className="dash-card__meta">
                        ...
                        <div>
                          <Clock size={14} /> {ws.durationMinutes || 90} mins
                        </div>
                        <div>
                          <User size={14} /> {ws.speakerName || 'Dr. Sarah Chen (Keynote)'}
                        </div>
                      </div>
            ...
            {liveWorkshop ? (
              <div className="dash-live-box">
                <div className="dash-live-box__header">
                  ...
                  <p style={{ color: '#94a3b8' }}>Led by: {liveWorkshop.speakerName}</p>
                </div>
              ...
              <div className="dash-grid-cards">
                {certificates.map((cert) => (
                  <div key={cert.id} className="dash-cert-card">
                    ...
                    <div className="dash-cert-code">ID: {cert.id}</div>
                    <button
            ...
            <form onSubmit={handleProfileSave} className="dash-profile-form">
              <div className="dash-form-row">
                <div className="dash-form-group">
                  ...
                  <input type="text" value={user?.name || ''} disabled className="dash-input-disabled" />
                </div>
                <div className="dash-form-group">
