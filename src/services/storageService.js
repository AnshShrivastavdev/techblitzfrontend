...
export function loginSpeaker(email, password) {
  const db = getDB();
  const user = db.users.find(
    (u) => u.email.toLowerCase() === email.toLowerCase().trim() && u.password === password && u.role === 'speaker'
  );
...
export function createWorkshop(data) {
  const db = getDB();
  const ws = { id: 'ws-' + generateId(), ...data };
...
export function registerForWorkshop(userId, workshopId) {
  const db = getDB();
...
export function unregisterFromWorkshop(userId, workshopId) {
  const db = getDB();
...
export function getAttendanceForUser(userId) {
  return getDB().attendanceLogs.filter((a) => a.userId === userId);
}

export function getAttendanceForWorkshop(workshopId) {
  return getDB().attendanceLogs.filter((a) => a.workshopId === workshopId);
}

export function pingAttendance(userId, workshopId) {
  const db = getDB();
...
export function getCertificateByNumber(certNumber) {
  return getDB().certificates.find((c) => c.certificateNumber === certNumber);
}

export function generateCertificates(workshopId) {
  const db = getDB();
...
export function issueCertificate(userId, recipientName, workshopId, workshopTitle) {
  const db = getDB();
  const exists = db.certificates.find(
...
export function getSpeakerByUserId(userId) {
  return getDB().speakers.find((s) => s.userId === userId);
}

export function updateSpeakerProfile(speakerId, updates) {
  const db = getDB();
...
export function addGalleryItem(data) {
  const db = getDB();
  const item = { id: 'gal-' + generateId(), ...data };
...
export function addFaq(data) {
  const db = getDB();
  const faq = { id: 'faq-' + generateId(), order: db.faqs.length + 1, published: true, ...data };
  db.faqs.push(faq);
  saveDB(db);
  return faq;
}
